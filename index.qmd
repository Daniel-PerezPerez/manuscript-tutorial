...
keywords:
  ...
  - Earthquakes
abstract: |
  In September 2021, a significant jump in seismic activity on the island of La Palma (Canary Islands, Spain) signaled the start of a volcanic crisis that still continues at the time of writing. Earthquake data is continually collected and published by the Instituto Geográphico Nacional (IGN). We aimed at describing alternative methods to describe the probability of a new eruption considering historical data. 
plain-language-summary: |
  Earthquake data for the island of La Palma from the September 2021 eruption is found in the database of the Instituto Geográphico Nacional (IGN). Current methods of statistical probability have limited advantages in the prediction of new eruptions.
key-points:
...
max_years = np.max(eruptions)
```

Based on data up to and including `{python} f"{max_years}"`, eruptions on La Palma happen every `{python} f"{avg_years_between_eruptions:.1f}"` years on average.
